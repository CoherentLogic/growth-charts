/*
*  v4wgcmw.js - Growth Charts Adult Men Weight
*
...
*  $Revision$
*/
// Growth Chart Adult Men Weight
var createMenWeightChart = function(patName, patRec, patDOB, weight) {
    ...
    for (row = 0;  row <= 820;  row += 10) { // Horizontal lines
        ...
        if (row % 25 == 0) {
            path.attr({stroke: "#888"});
            paper.text(666 + xoffset, row + yoffset, "" + (((850 - row) / 5) +0) );
        } else {
            path.attr({stroke: "#BBB"});
        }
    } 
    ...
    text(91.2*12,182.5,"Today: " + date);
    text(90.7*12,179,"Chart: Adult Male Weight");
